...
5. Выше добавьте заголовок `План закупок` - объедините ячейки и выровняйте по центру (поставить галочку в `Формат ячеек \ Выравнивание \ Объединить ячейки`)  
6. Ниже названий, в столбцы 5-й строки (начиная с ячейки E5) добавьте строку  
`Клей Момент`, `шт`, `19`, `30,1`  
Добавьте еще строки
```
...
2. Нажмите плюс (на диаграмме, справа сверху) и измените подписи на Значения - чтобы были видны суммы для каждого товара  
`+ \ Подписи к данным \ Дополнительные параметры \ Значения`
3. Измените название диаграммы - добавив ссылку на название таблицы (ячейка E3)
